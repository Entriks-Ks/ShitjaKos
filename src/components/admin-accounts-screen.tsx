...
export async function AdminAccountsScreen({
  ...
  const basePath = kind === "user" ? "/admin/users" : "/admin/businesses";
  ...
  return (
    <>
      <header className="workspace-heading">
        ...
        <p className="muted">Search accounts and manage suspension status.</p>
      </header>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-stone-200">
              ...
              <th className="p-3">{kind === "user" ? "Type / role" : "Approval"}</th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-stone-100 align-top">
                ...
                <td className="p-3">
                  <span className={row.suspended ? "text-red-700" : "text-green-700"}>
